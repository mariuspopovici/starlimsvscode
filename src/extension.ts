...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (!rootPath) {
    ...
    if (newRootPath) {
      ...
      vscode.window.showErrorMessage("Please configure STARLIMS root path in extension settings.");
      return;
    }
  }
  ...
  vscode.commands.registerCommand("STARLIMS.GetLocal", async (item: TreeEnterpriseItem | any) => {
    // get local copy of the item
  ...
  vscode.commands.registerCommand("STARLIMS.RunScript", async (item: TreeEnterpriseItem | any) => {
  ...
  vscode.commands.registerCommand("STARLIMS.Compare", async (uri: vscode.Uri) => {
    ...
    if (localUri) {
      if (config.has("rootPath")) {
        const rootPath: string = path.join(config.get("rootPath") as string, SLVSCODE_FOLDER);
  ...
  vscode.commands.registerCommand("STARLIMS.ShowTreeView", async (item: TreeEnterpriseItem) => {
    enterpriseProvider.refresh();
  });
  ...
  vscode.commands.registerCommand("STARLIMS.ClearLog", async (item: TreeEnterpriseItem) => {
  ...
  vscode.commands.registerCommand("STARLIMS.Search", async (item: TreeEnterpriseItem) => {
    // ask for search text
  ...
  vscode.commands.registerCommand("STARLIMS.OpenForm", async (item: TreeEnterpriseItem | any) => {
    let remoteUri: string = "";
    const isTreeCommand = item instanceof TreeEnterpriseItem;
  ...
  vscode.commands.registerCommand("STARLIMS.Add", async (item: TreeEnterpriseItem) => {
      ...
      // add item to application
        ...
        switch (selectedItemType) {
          ...
          case "DataSources":
            ...
            itemLanguage = await vscode.window.showQuickPick(["SSL", "SQL"], {
  ...
  vscode.commands.registerCommand("STARLIMS.Delete", async (item: TreeEnterpriseItem) => {
    if (selectedItem === undefined) {
  ...
  vscode.commands.registerCommand("STARLIMS.RunDataSource", async (item: TreeEnterpriseItem | any) => {
    let remoteUri: string = "";
    // commands can originate from the enterprise tree or from an open editor window
    const isTreeCommand = item instanceof TreeEnterpriseItem;
  ...
  vscode.commands.registerCommand("STARLIMS.OpenXFDForm", async (item: TreeEnterpriseItem | any) => {
    ...
    outputChannel.appendLine(`${new Date().toLocaleString()} Launching remote form at URI: ${remoteUri}`);

    const result = await enterpriseService.runXFDForm(remoteUri.toString());
    if (result) {
      outputChannel.appendLine(
        "Launched form successfully. Please wait while the STARLIMS HTML bridge completes the request."
      );
      outputChannel.show();
    }
  });
